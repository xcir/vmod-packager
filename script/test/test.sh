...
./vmod-packager.sh -t -d focal -k -r test-varnish-cache src/test-varnish-modules71;      test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 3
...
./vmod-packager.sh -t -k -d focal src/test-varnish-modules71 src/test-libvmod-digest70;   test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 4
