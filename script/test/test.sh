...
# clean
sudo rm -rf src/test-varnish-modules60* src/test-varnish-modules70* src/test-libvmod-digest71*
sudo rm -rf pkgs/*/test-varnish-modules60 pkgs/*/test-varnish-modules70 pkgs/*/test-varnish-modules71
sudo rm -rf pkgs/*/test-libvmod-digest60 pkgs/*/test-libvmod-digest71
...
mkdir -p src/test-varnish-modules71
mkdir -p src/test-libvmod-digest71
mkdir -p src/test-libvdp-pesi71
mkdir -p src/test-vmod-reqwest

mkdir -p varnish/test-varnish-cache
curl -sL https://varnish-cache.org/_downloads/varnish-7.1.0.tgz | tar zx -C varnish/test-varnish-cache --strip-components 1
...
curl -sL https://github.com/varnish/varnish-modules/archive/refs/heads/7.1.tar.gz       | tar zx -C test-varnish-modules71 --strip-components 1
...
curl -sL https://github.com/varnish/libvmod-digest/archive/1793bea9e9b7c7dce4d8df82397d22ab9fa296f0.tar.gz | tar zx -C test-libvmod-digest71 --strip-components 1
...
(
    cd test-libvmod-digest71
    patch -p0 < ../../script/test/digest.patch
)

# copy custom script
cp ${VRD}/sample-src/libvmod-digest_init.sh ${VRD}/src/test-libvmod-digest71_init.sh
cp ${VRD}/sample-src/libvmod-digest_env.sh  ${VRD}/src/test-libvmod-digest71_env.sh
...
#ubuntu
./vmod-packager.sh -t -d jammy -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1

./vmod-packager.sh -t -d focal -v 6.0.8 test-varnish-modules60; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1
./vmod-packager.sh -t -d focal -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1

./vmod-packager.sh -t -d bionic -v 6.0.8 test-varnish-modules60; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1
./vmod-packager.sh -t -d bionic -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1

#debian
./vmod-packager.sh -t -d buster -v 6.0.8 test-varnish-modules60; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2
./vmod-packager.sh -t -d buster -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2

./vmod-packager.sh -t -d bullseye -v 6.0.8 test-varnish-modules60; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2
./vmod-packager.sh -t -d bullseye -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2

#centos
./vmod-packager.sh -t -d centos8 -v 6.0.8 test-varnish-modules60; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2
./vmod-packager.sh -t -d centos8 -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2

#centos-stream
./vmod-packager.sh -t -d centos_stream8 -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2
./vmod-packager.sh -t -d centos_stream9 -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2

# arch
./vmod-packager.sh -t -d arch -v 7.1.0 test-varnish-modules71; test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1
...
# libvmod-digest
./vmod-packager.sh -t -d jammy -v 7.1.0 src/test-libvmod-digest71;      test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1
./vmod-packager.sh -t -d centos8 -v 7.1.0 src/test-libvmod-digest71;    test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2
./vmod-packager.sh -t -d arch -v 7.1.0 src/test-libvmod-digest71;       test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 1
...
./vmod-packager.sh -t -d jammy -k -r test-varnish-cache src/test-varnish-modules71;      test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 3
./vmod-packager.sh -t -d centos8 -k -r test-varnish-cache src/test-varnish-modules71;    test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 5
./vmod-packager.sh -t -d arch -k -r test-varnish-cache src/test-varnish-modules71/;      test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 2

# multi vmod with varnish
./vmod-packager.sh -t -k -d jammy src/test-varnish-modules71 src/test-libvmod-digest71;   test $(egrep "\\.(deb|rpm|zst)" tmp/vmp_*.log|wc -l) -eq 4
