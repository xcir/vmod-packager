...
./vmod-packager.sh -t -d focal -v 7.0.0 src/test-libvmod-digest70
if [ $? -eq 0 ]; then exit 1; fi
./vmod-packager.sh -t -d centos8 -v 7.0.0 src/test-libvmod-digest70
if [ $? -eq 0 ]; then exit 1; fi
...
./vmod-packager.sh -t -d focal -v 7.0.0 src/test-libvdp-pesi70 && ls ${VRD}/pkgs/debs/test-libvdp-pesi70/test-libvdp-pesi70_140.0.1~focal-1_amd64.deb
if [ $? -eq 1 ]; then exit 1; fi
./vmod-packager.sh -t -d centos8 -v 7.0.0 src/test-libvdp-pesi70 && ls ${VRD}/pkgs/rpms/test-libvdp-pesi70/test-libvdp-pesi70-140.0.1-1.el8.x86_64.rpm
if [ $? -eq 1 ]; then exit 1; fi
